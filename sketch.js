...
function draw() {
  ...
  chains.display();
  imageMode(CENTER);
  image(image,ball.position.x,ball.position.y,40,40);
}
...
function keyPressed(){
	if(keyCode === 32)
	{
		Matter.Body.setPosition(playingobj.body,{x:235, y:420});
		chains.attach(playingobj.body);
	}
}
